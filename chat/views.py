    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if contact_user == request.user:
                return JsonResponse({'success': False, 'error': "You can't add yourself"})
            Contact.objects.get_or_create(owner=request.user, contact=contact_user)
...
def search_users(request):
    ...
    blocked_ids = BlockedUser.objects.filter(user=request.user).values_list('blocked_id', flat=True)
    existing_contact_ids = Contact.objects.filter(owner=request.user).values_list('contact_id', flat=True)
